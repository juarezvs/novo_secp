...
export async function createPermissao(formData: FormData) {
  ...
  const recurso = String(formData.get('recurso') ?? '').trim().toLowerCase()
  const acao = String(formData.get('acao') ?? '').trim().toLowerCase()
  const escopo = String(formData.get('escopo') ?? '').trim().toLowerCase()
